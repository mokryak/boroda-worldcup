...
2. Импортируйте CSV из папки `seed/` как отдельные листы:
   ...
   - `Participants`
   - `Predictions`
   - `Reviews` создастся автоматически после обновления Apps Script или первого обзора
...
Live-счет подключен через TheSportsDB, без API-ключа во фронтенде или git:

1. Сохраните `Code.js` и сделайте `Deploy -> Manage deployments -> Edit -> New version -> Deploy`.
2. Запустите `installLiveScoreTrigger` один раз, если хотите, чтобы итоговый счет записывался даже когда сайт никто не открыл.

Apps Script дергает TheSportsDB только если есть матч в live-окне: от 15 минут до старта и до 4 часов после старта, при этом в `Matches` еще нет финального счета. Ответ кэшируется примерно на 5 минут. Когда API возвращает завершенный матч, Apps Script записывает итог в `Matches.actual_home`, `Matches.actual_away` и `Matches.status = complete`; после этого матч больше не запрашивается у внешнего API.

### Ежедневные обзоры

Обзоры хранятся в листе `Reviews` и читаются сайтом через `GET <WEB_APP_URL>?action=reviews`. Последний обзор показывается на главной, полный список доступен в разделе `/reviews`.

Для публикации нужен `adminToken` в Apps Script `Project Settings -> Script Properties`. Токен не хранится в git. После подготовки Markdown-файла обзор публикуется так:
...
npm run publish:review -- \
  ...
  --body /tmp/boroda-review.md
```

Подробная инструкция для ежедневного агента: [`docs/daily-review-agent.md`](docs/daily-review-agent.md).

Для ежедневного запуска в Codex сначала соберите briefing:

```bash
npm run review:context
```

Команда читает `.env.local`, забирает `state`/`reviews`, считает очки, синхронизирует опубликованные обзоры в `content/reviews/published/` и пишет `tmp/daily-review-context.md`. После этого агент проверяет реальные матчи в веб-источниках и сохраняет черновик в `content/reviews/drafts/`. Публикуем только после ручного подтверждения:

```bash
npm run publish:review -- --body content/reviews/drafts/YYYY-MM-DD.md
```
